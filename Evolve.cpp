...
void Evolve_element::integrate_numerical_flux(){
    ...
    double integral_side_1, integral_side_2, integral_side_3;

    // loop over all the interior nodes of this element
    for (int i = 0; i < ( this->p + 1 ) * ( this->p + 2 ) / 2; ++i) {    
        // loop over hidrodynamics index
        for (int k = 0; k < 4; ++k) {
            //initilize variable that contains the integral over sides 1, 2 and 3
            integral_side_1 = 0.0;
            integral_side_2 = 0.0;
            integral_side_3 = 0.0;
            ...
            for (int j = 0; j < num_gauss_quad_pnts; ++j) {
                //                 +=           w(xi')           * (                    phi(xi')                      *             \hat_{f}              )
                integral_side_1 += this->gau_integ_line[j][1] * ( this->plus_phi_in_quadrature_points_side_1[i][j] * this->numerical_flux_side_1[j][k] );
                integral_side_2 += this->gau_integ_line[j][1] * ( this->plus_phi_in_quadrature_points_side_2[i][j] * this->numerical_flux_side_2[j][k] );
                integral_side_3 += this->gau_integ_line[j][1] * ( this->plus_phi_in_quadrature_points_side_3[i][j] * this->numerical_flux_side_3[j][k] );
            }
            // multiply integral by the jacobian : this is lenght of the integration path
            integral_side_1 *= this->this_element->sides_lenght[0];
            integral_side_2 *= this->this_element->sides_lenght[1];
            integral_side_3 *= this->this_element->sides_lenght[2];

            // sum the integrals over the three side of the element
            this->DG_numerical_flux_integration[i][k] = integral_side_1 + integral_side_2 + integral_side_3;
